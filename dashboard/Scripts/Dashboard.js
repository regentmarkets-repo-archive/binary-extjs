...
Ext.app.Dashboard = function (id, currentUserID, mode)
{
	...
	if (mode == "local")
	{
		...
		componentsProxy =
		{
			...
			url: '../../Scripts/dataComponents.json'
		}		
	}
	...
	var dashboardComponentsStore = new Ext.data.JsonStore(
	{
		...
		fields: ['DashboardID', 'ComponentID', 'CurrentSettings', 'DashletID', 'ID'],
	...
	var componentsStore = new Ext.data.JsonStore(
	{
		...
		fields: ['Name', 'ID', 'IconUrl', 'ComponentID', 'Settings', 'SettingsConfig'],
	...
	this.addComponent = function (component)
	{
		...
		var record = dashboardComponentsStore.add(
		{
			...
			CurrentSettings: component.data.Settings,//test
			ID: 0
		});
	...
	{
		cmp.component.CurrentSettings.ID = cmp.component.get("ID");
		Ext.Ajax.request(
		{
			url: "Dashboard/UpdateComponentSettings",
			params: cmp.component.CurrentSettings,
			success: function (response)
			{
				cmp.setTitle(cmp.component.CurrentSettings.Title);
			}
		});
	};
	...
	{
		cmp.component.CurrentSettings.Collapsed = true;
		saveComponentState(cmp);
	});
	mediator.on("componentExpanded", function (cmp)
	{
		cmp.component.CurrentSettings.Collapsed = false;
		saveComponentState(cmp);
	});
};
