...
Ext.app.Dashboard.LayoutFactory = function (dashboard)
{
	...
	this.addComponentInternal = function (portalPanel, component)
	{
		...
		component.CurrentSettings = component.data.Settings || component.data.CurrentSettings;

		var componentConfig =
		{
			title: component.CurrentSettings.Title,
			xtype: 'portlet',
			component: component,
			collapsed: component.CurrentSettings.Collapsed,
			tools:
			[
				{
					xtype: 'tool',
					...
					handler: function (e, target, panelHeader, tool)
					{
						...
						var sourceConfig = dashboard.getComponents().findRecord("ID", cmp.component.get("ComponentID")).data.SettingsConfig;
						//dashboard.getComponents().findRecord("ComponentID", cmp.component.get("ComponentID"))
						var settingsCopy = Ext.clone(cmp.component.CurrentSettings);
						var w = new Ext.window.Window(
						{
							...
							[
								{
									xtype: 'propertygrid',
									source: cmp.component.CurrentSettings,
							...
							[
								...
								},
								{
									...
									handler: function ()
									{
										cmp.component.CurrentSettings = settingsCopy;
										this.up("window").close();
									}
								}
							]
		...
		var componentUrl = component.CurrentSettings.Url;
		if (componentUrl.indexOf('?') == -1) componentUrl += '?';
		componentUrl += Ext.Object.toQueryString(Ext.applyIf({ CurrentSettings: null }, component.data));

		if (component.CurrentSettings.IFrame)
		{
			var html = Ext.String.format('<iframe src="{0}" style="width:100%;height:{1}; border:0" onload="Ext.app.Dashboard.LayoutFactory.IFrameLoaded(\'{2}\')"></iframe>',
				componentUrl,
				component.CurrentSettings.Height,
